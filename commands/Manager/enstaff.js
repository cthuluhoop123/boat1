...
module.exports.run = async (bot, message, args) => {
    ...
    GetRef.get().then(function (doc) {
        if (!doc.exists) {
            ...
            return;
        } else if (doc.data.staff == true) {
            message.channel.send(`${emotes.Error} **${bot.guilds.get(serverID).name} Is already a staff guild.**`);
            return;
        } else {
            db.collection("guilds").doc(message.guild.id).update({
                ...
                .then(() => {
                    message.channel.send(`${emotes.Success} **Enabled Staff version for \`${bot.guilds.get(serverID).name}\`**`);
                })
...
module.exports.command = {
    ...
    aliases: [],
    usable: "Owner",
    description: "Enables Stan Staff on a server.",
    usage: "enstaff <SERVERID>",
    category: "Owner",
    enabled: true
};